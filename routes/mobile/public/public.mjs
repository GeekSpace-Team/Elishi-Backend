...
import {
  verifyTokenWithNext,
  verifyUserToken,
} from "../../../modules/auth/token.mjs";
import {
  addProduct,
  addProductImage,
  deleteFirstImages,
  deleteImageById,
  deleteNotFirstImages,
  deleteProductImages,
  delete_product,
  getOldImages,
  update_product_query_user,
  update_product_query_user_with_status,
} from "../../../modules/constant/admin_query.mjs";
...
import {
  ...
  searchQuery,
  vipUser,
} from "../../../modules/constant/user_queries.mjs";
import { db } from "../../../modules/database/connection.mjs";
import {
  defaultMessage,
  errorMessage,
  message,
} from "../../../modules/message.mjs";
import {
  badRequest,
  reachLimit,
  response,
...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    if (
      file.fieldname === first_image_large ||
      file.fieldname === sliders_large
    )
      cb(null, folder_large);
    else if (
      file.fieldname === first_image_small ||
      file.fieldname === sliders_small
    )
...
publicRouter.get("/get-congratulations", (req, res) => {
  if (typeof req.query.page === "undefined" || req.query.page == null) {
    ...
    if (
      typeof req.query.holiday !== "undefined" &&
      req.query.holiday != null &&
      req.query.holiday != ""
    ) {
...
publicRouter.get("/get-home", verifyTokenWithNext, (req, res) => {
  ...
  if (
    ...
    typeof req.query.device !== "undefined" &&
  ...
  db.query(
    `${getBanners} ${getMainCategory} ${getAds} ${vipUser} ${getEvents} ${format(
      getNewProducts,
      userId
    )} ${format(
      getTrendProducts,
      userId
    )} ${getCollections} ${getDeviceVersion}`
  )
    .then((results) => {
      ...
      if (collections.length > 0) {
        ...
        res.send(
          response(false, defaultMessage, {
            ...
            vip_users: results[3].rows,
            events: results[4].rows,
            newProducts: addDefaultImage(results[5].rows),
            trendProducts: addDefaultImage(results[6].rows),
            deviceVersion: results[8].rows,
            eventProducts: null,
          })
        );
...
publicRouter.get(
  ...
  async (req, res) => {
    ...
    if (
      typeof req.query === "undefined" ||
      typeof req.query == null ||
      typeof req.query.p_id === "undefined" ||
      req.query.p_id == null
    ) {
      badRequest(req, res);
      return;
    ...
    await db
      .query(getProductById, [userId, product_id])
      .then((result) => {
...
publicRouter.post("/get-products", verifyTokenWithNext, async (req, res) => {
  ...
  if (
    typeof req.body.sub_category !== "undefined" &&
    req.body.sub_category != null &&
    req.body.sub_category.length > 0
  ) {
  ...
  if (typeof req.body.category !== "undefined" && req.body.category != null) {
    await db
      .query(getSubCategoryCon, [req.body.category])
      .then((result) => {
  ...
  let priceQuery = ``;
  if (
    typeof req.body.min !== "undefined" &&
    req.body.min != null &&
    typeof req.body.max !== "undefined" &&
    req.body.max != null
  ) {
  ...
  console.log(query);
  await db
    .query(query, [userId, limit, page])
    .then((result) => {
      res.json(response(false, defaultMessage(), addDefaultImage(result.rows)));
    })
    .catch((err) => {
      badRequest(req, res);
    });
...
const createSmallImage = async (
  ...
  sharp(filename)
    ...
    .resize(200)
    .toFile(
      "public/uploads/product/small/" + uniqueSuffix + ".png",
      (err, data) => {
        if (err) {
          console.error(err);
          return;
...
const addProductFunction = async (req, res) => {
  ...
  const { name, price, size, sub_category, phone_number, description } =
    req.body;
...
publicRouter.post("/get-verify-code", verifyUserToken, (req, res) => {
  if (
    ...
    !startsWith(
      req.body.phoneNumber,
      "+9936" || typeof req.body.type === "undefined" || req.body.type == null
    )
  ) {
    console.log("Error 1");
    badRequest(req, res);
...
publicRouter.post(
  ...
  checkFolder_large,
  checkFolder_small,
  uploader.single(first_image_large),
  resizeImage
);
...
const updateProduct = (req, res) => {
  ...
  userId = req.user.user.id;
  const { id, name, price, size, sub_category, description, isDangerous } =
    req.body;
            ...
            db.query(format(addProductImage, imgValues))
              .then((imageResult) => {
                if (typeof imageResult !== "undefined") {
                  ...
                  res.json(
                    response(false, defaultMessage(), { product: result.rows })
                  );
                  res.end();
                }
              })
